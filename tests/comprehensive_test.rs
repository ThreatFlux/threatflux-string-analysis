...
fn test_string_tracking_edge_cases() {
    ...
    assert!(result.is_ok(), "String with special characters should be handled");
...
fn test_batch_string_tracking() {
    ...
    tracker.track_strings_from_results(&strings, "/test/batch", "batch_hash", "batch_tool")
        .unwrap();
...
fn test_context_preservation() {
    ...
    let contexts = vec![
        ("http://example.com", StringContext::Url { protocol: Some("http".to_string()) }),
        ("/usr/bin/test", StringContext::FilePath),
        ("HKEY_LOCAL_MACHINE", StringContext::RegistryKey),
        ("CreateProcessA", StringContext::ApiCall { library: Some("kernel32.dll".to_string()) }),
        ("192.168.1.1", StringContext::IpAddress { version: Some(4) }),
        ("test@example.com", StringContext::Email),
        ("secret_password", StringContext::Credential { credential_type: Some("password".to_string()) }),
        ("random_string", StringContext::FileString { offset: Some(1024) }),
    ];
...
fn test_filtering_capabilities() {
    ...
    let test_strings = vec![
        ...
        ("very_long_string_that_exceeds_normal_limits_for_testing_purposes", 1), // Long
...
fn test_related_strings() {
    ...
    for string in &related_strings {
        tracker.track_string(
            ...
            "malware_hash",
            "pe_parser",
            StringContext::Import { library: Some(string.to_string()) },
        ).unwrap();
    }
...
fn test_entropy_calculation() {
    ...
    for (string, description) in entropy_tests {
        ...
        assert!(details.entropy >= 0.0, "Entropy should be non-negative for: {}", description);
        assert!(details.entropy <= 8.0, "Entropy should not exceed theoretical maximum for: {}", description);
    }
}
...
fn test_suspicious_detection_patterns() {
    ...
    let suspicious_patterns = vec![
        ...
        ("powershell.exe -EncodedCommand", "PowerShell encoded command"),
    ...
    for suspicious_string in &suspicious_stats.suspicious_strings {
        ...
        assert!(is_in_suspicious_patterns, "String '{}' should be marked as suspicious", suspicious_string);
    }
}
...
fn test_performance_with_large_dataset() {
    ...
    assert!(tracking_time.as_secs() < 10, "Tracking {} strings should complete in under 10 seconds", num_strings);
    ...
    assert!(search_time.as_millis() < 1000, "Search should complete in under 1 second");
    assert!(!search_results.is_empty(), "Search should find matching strings");
}
