...
fn test_string_context_variants() {
    ...
    let contexts = vec![
        ...
        StringContext::ApiCall { library: Some("kernel32.dll".to_string()) },
        ...
        StringContext::Email,
        StringContext::Credential { credential_type: Some("password".to_string()) },
        StringContext::Credential { credential_type: None },
        StringContext::Import { library: Some("msvcrt.dll".to_string()) },
...
fn test_string_filter_combinations() {
    ...
    let test_strings = vec![
        ...
        ("very_long_string_for_comprehensive_testing_purposes", 1, false), // Long, low occurrence
    ...
    for (string, count, _is_suspicious) in &test_strings {
        for i in 0..*count {
            tracker.track_string(
                ...
                &format!("hash_{}", i),
                "filter_tool",
                StringContext::FileString { offset: Some(i * 10) },
            ).unwrap();
        }
    }
...
fn test_nonexistent_string_details() {
    ...
    assert!(result.is_err(), "Should return error for nonexistent string");
}
...
fn test_search_edge_cases() {
    ...
    assert!(no_match_results.is_empty(), "Non-matching query should return no results");
    
    // Test zero limit
    let zero_limit_results = tracker.search_strings("app", 0);
    assert!(zero_limit_results.is_empty(), "Zero limit should return no results");
    
    // Test very large limit
    let large_limit_results = tracker.search_strings("app", 1000);
    assert!(large_limit_results.len() <= test_strings.len(), "Should not exceed available matches");
    
    // Test single character search
    let single_char_results = tracker.search_strings("a", 10);
    assert!(!single_char_results.is_empty(), "Single character should match");
}
...
fn test_related_strings_edge_cases() {
    ...
    assert!(nonexistent_related.is_empty(), "Should return empty for nonexistent string");
    ...
    assert!(zero_limit_related.is_empty(), "Zero limit should return no results");
}
...
fn test_batch_operations() {
    ...
    // Test empty batch
    let empty_result = tracker.track_strings_from_results(&[], "/test/empty", "empty_hash", "empty_tool");
    ...
    tracker.track_strings_from_results(&duplicate_strings, "/test/duplicates", "dup_hash", "dup_tool")
        .unwrap();
...
fn test_error_handling() {
    ...
    assert!(empty_tool_result.is_ok(), "Empty tool name should be handled");
}
...
fn test_categorization_accuracy() {
    ...
    for (string, expected_categories) in categorization_tests {
        ...
        let has_expected_category = expected_categories.iter().any(|expected| {
            details.categories.iter().any(|actual| actual.contains(expected))
        });
        
        assert!(has_expected_category, 
...
fn test_entropy_calculation_consistency() {
    ...
    for (string, description) in entropy_tests {
        if !string.is_empty() {
            ...
            assert!(details.entropy >= 0.0, "{}: Entropy should be non-negative", description);
            assert!(details.entropy <= 8.0, "{}: Entropy should not exceed maximum", description);
            
            // Test entropy consistency (same string should have same entropy)
            let details2 = tracker.get_string_details(string).unwrap();
            assert_eq!(details.entropy, details2.entropy, "{}: Entropy should be consistent", description);
        }
    }
}